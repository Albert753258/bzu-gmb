...
tput setaf 2; echo "Установка открытой утилиты Goverlay от benjamimgois [https://github.com/benjamimgois/goverlay]. Установка утилиты осуществлыется через репозиторий: [sudo add-apt-repository ppa:flexiondotorg/mangohud]  Версия скрипта 1.2, автор: Яцына М.А."
tput sgr0
...
echo "${pass_user}" | sudo -S apt install -f -y goverlay || let "error += 1"
fi
...
goverlay & sleep 12;echo "${pass_user}" | sudo -S killall goverlay
