...
echo "${pass_user}" | sudo -S emerge --update media-video/simplescreenrecorder || let "error += 1"
#формируем информацию о том что в итоге установили и показываем в терминал
app_name="simplescreenrecorder"
if [ 'eix-installed -a | grep media-video/simplescreenrecorder' != "" ];then
tput setaf 2; echo "${app_name}:installed"
tput sgr0
echo "Testing:${app_name}"
# 5 секунд теста программы
simplescreenrecorder & sleep 5;echo "${pass_user}" | sudo -S killall simplescreenrecorder
