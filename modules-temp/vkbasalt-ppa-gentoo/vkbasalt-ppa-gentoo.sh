...
DadSchoorse [https://github.com/DadSchoorse/vkBasalt]. Установка утилиты осуществлыется через репозиторий: [sudo add-apt-repository ppa:flexiondotorg/mangohud]  Версия скрипта 1.2, автор: Яцына М.А."
tput sgr0
...
sudo -S emerge --update app-misc/vkBasalt || let "error += 1"
cd
...
MANGOHUD_DLSYM=1 mangohud MANGOHUD_CONFIG=wine,benchmark_percentiles,gpu_name,cpu_temp,gpu_temp,vram,ram,font_size=22,vulkan_driver,arch,gpu_core_clock,gpu_mem_clock,position=top-left ENABLE_VKBASALT=1 glxgears -geometry 800x600+0+0 | sleep 5 | exit 0
